// Uniformity probe p18_after_varying_loop: WGSL uniformity prepass tint-evidence
...
// RULE EVIDENCED:
//   the code AFTER a varying-bound loop RECONVERGES: every invocation leaves the
//   loop at the same join, so the post-loop sample stays implicit; contrast the
//   in-loop sample of p09
// CITED BY: the post-loop reconvergence keep
...
fn main(@builtin(position) fc: vec4f) -> @location(0) vec4f {
    var c = vec4f(0.0);
    let n = i32(fc.x);
    for (var i = 0; i < n; i = i + 1) { c = c + vec4f(f32(i)) * 0.001; }

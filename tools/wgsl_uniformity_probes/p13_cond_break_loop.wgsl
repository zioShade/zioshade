// Uniformity probe p13_cond_break_loop: WGSL uniformity prepass tint-evidence
...
// RULE EVIDENCED:
//   a conditional break inside a loop leaves the post-break code running for a
//   SUBSET of invocations, so the in-loop sample after the break check is
//   non-uniform
// CITED BY: the subset-merge list (conditional break)
...
fn main(@builtin(position) fc: vec4f) -> @location(0) vec4f {
    ...
    for (var i = 0; i < 4; i = i + 1) {
        if (fc.x > 8.0) { break; }
        c = c + textureSample(tex, smp, fc.xy);
    }
    return c;
}

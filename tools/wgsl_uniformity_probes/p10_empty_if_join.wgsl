// Uniformity probe p10_empty_if_join: WGSL uniformity prepass tint-evidence
...
// RULE EVIDENCED:
//   an if on a NON-uniform condition whose arms both complete RECONVERGES at the
//   merge: every invocation that entered reaches the sample, so it stays implicit
// CITED BY: the postdominance rule (the reconvergence keep)

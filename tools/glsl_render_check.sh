...
check_one() {
  ...
  # MATCH under precise-FP => a contraction/rounding edge (EDGE), not a real divergence.
  # Mirrors frag_oracle_check.sh / prove_opt.sh / prove_naga.sh.
...
for k in MATCH "EDGE(fast-math-fp)" DIFFER skip-binding skip-glslang skip-crossmsl skip-zglsl skip-zglslang skip-zcrossmsl skip-render; do
  echo "  $k: ${C[$k]:-0}"
done

...
const wgsl_uniformity = @import("wgsl_uniformity.zig");
...
// OpTerminateInvocation's raw opcode (4416) is matched by
// wgsl_uniformity.zig's `isTerminateInvocation`: the uniformity prepass is
// its only consumer.
...
/// Record the uniformity prepass's non-convergence (into the threadlocal
/// detail), then return the honest error. The prepass module
/// (wgsl_uniformity.zig) has no detail channel of its own; this backend owns
/// the message text and the round count it cites (wgsl_uniformity.max_rounds).
fn recordUniformityPrepassDidNotConverge() error{UniformityAnalysisDidNotConverge} {
    last_error_detail = std.fmt.bufPrint(
        &last_error_detail_buf,
        "the WGSL uniformity prepass did not converge in {d} rounds. Every component of that fixpoint only moves DOWN, so this is an INTERNAL INVARIANT failure in zioshade, not a problem with your shader. Workaround: none; please file a bug against zioshade with the shader (or the SPIR-V) that produced this.",
        .{wgsl_uniformity.max_rounds},
    ) catch null;
...
/// id of the innermost (non-array) element type. Moved to
/// spirv_cross_common.zig (issue #691, shared with wgsl_uniformity.zig); its
/// doc carries the BufferBlock/array-of-SSBO context and the depth-cap note.
...
/// spirv_cross_common.zig by issue #691 (this, the MSL and the GLSL backend
...
pub fn spirvToWGSL(alloc: std.mem.Allocator, spirv_words_in: []const u32, options: WgslCompileOptions) ![]const u8 {
    ...
    // that sits in non-uniform control flow (see wgsl_uniformity.zig's
    ...
    const nonuniform_gated = wgsl_uniformity.computeNonuniformGatedBuiltinIds(arena, &module, &decorations) catch |e| switch (e) {
        error.OutOfMemory => return error.OutOfMemory,
        // The prepass has no detail channel of its own; record the invariant
        // failure here, into this backend's threadlocal detail.
        error.UniformityAnalysisDidNotConverge => return recordUniformityPrepassDidNotConverge(),
    };
...
// #wgsl-uniformity-8k2: the uniformity prepass (which implicit-Lod samples and
// which derivatives sit in non-uniform control flow, and therefore get lowered
// to the explicit-Level form / refused at emission) lives in
// wgsl_uniformity.zig since issue #691. This file consumes its one result map
// (`nonuniform_gated`, built at the spirvToWGSL prepass call below) at every
// gated builtin.

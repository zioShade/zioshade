...
pub fn deadLoopElim(alloc: std.mem.Allocator, words: []const u32) error{OutOfMemory}![]const u32 {
    ...
    for (loops.items, 0..) |loop_info, li| {
        ...
        // Phase 2.5: check if loop stores to function-local vars that flow to output/buffer stores
        // This catches cases like: sum += texture(tex, ...) inside a loop, where sum is
        // later stored to an output variable.
        var func_var_stored_in_loop = std.DynamicBitSet.initEmpty(alloc, bound) catch continue;
        defer func_var_stored_in_loop.deinit();
        in_loop = false;
        pos = 5;
        while (pos < words.len) {
            const hdr = words[pos]; const wc: u32 = hdr >> 16; const opcode: u16 = @truncate(hdr & 0xFFFF);
            ...
            if (in_loop and opcode == 62 and wc >= 3) { // OpStore
                const ptr_id = words[pos + 1];
                if (func_vars.contains(ptr_id) and ptr_id < bound) {
                    func_var_stored_in_loop.set(ptr_id);
                }
            }
            pos += wc;
        }
        if (func_var_stored_in_loop.count() == 0) {
            // No function-local vars stored in loop — skip to Phase 3
        } else {
            // Build transitive closure: AccessChains from stored vars
            var escaping_vars = std.DynamicBitSet.initEmpty(alloc, bound) catch continue;
            defer escaping_vars.deinit();
            var bsi: u32 = 0;
            while (bsi < bound) : (bsi += 1) {
                if (func_var_stored_in_loop.isSet(bsi)) escaping_vars.set(bsi);
            }
            for (0..2) |_| {
                var sp: u32 = 5;
                while (sp < words.len) {
                    ...
                    if (sop == 65 and swc >= 5) {
                        const base = words[sp + 3];
                        const result_ac = words[sp + 2];
                        if (base < bound and escaping_vars.isSet(base) and result_ac < bound) {
                            escaping_vars.set(result_ac);
                        }
                    }
                    sp += swc;
                }
            }
            var stored_var_reaches_output = false;
            pos = 5;
            while (pos < words.len) {
                const hdr = words[pos]; const wc: u32 = hdr >> 16; const opcode: u16 = @truncate(hdr & 0xFFFF);
                if (wc == 0) break;
                if (opcode == 62 and wc >= 3) { // OpStore
                    const ptr_id = words[pos + 1];
                    if (!func_vars.contains(ptr_id)) {
                        const val_id = words[pos + 2];
                        var sp2: u32 = 5;
                        while (sp2 < words.len) {
                            const h2 = words[sp2]; const w2: u32 = h2 >> 16; const op2: u16 = @truncate(h2 & 0xFFFF);
                            if (w2 == 0) break;
                            if (op2 == 61 and w2 >= 4 and words[sp2 + 2] == val_id) {
                                if (words[sp2 + 3] < bound and escaping_vars.isSet(words[sp2 + 3])) {
                                    stored_var_reaches_output = true;
                                }
                                break;
                            }
                            sp2 += w2;
                ...
                if (stored_var_reaches_output) break;
                pos += wc;
            }
            if (stored_var_reaches_output) {
                // Safety: verify loop structure is intact (continue and merge labels exist)
                const continue_id = words[loop_info.header_pos + 2];
                var cont_exists = false;
                var merge_exists = false;
                var sp3: u32 = 5;
                while (sp3 < words.len) {
                    const h3 = words[sp3]; const w3: u32 = h3 >> 16;
                    if (w3 == 0) break;
                    if ((@as(u16, @truncate(h3 & 0xFFFF)) == 248) and w3 >= 2) {
                        if (words[sp3 + 1] == continue_id) cont_exists = true;
                        if (words[sp3 + 1] == loop_info.merge_id) merge_exists = true;
                    }
                    sp3 += w3;
                }
                if (cont_exists and merge_exists) continue; // preserve this loop
            }
        }
        ...
        if (wc >= 5) {
            ...
            if (result_id > 0 and result_id < bound and a > 0 and a < bound and b > 0 and b < bound) {
                switch (opcode) {
                    ...
                    142 => { // OpVectorTimesScalar: vec * 1.0 = vec
                        if (float_one_ids.isSet(b)) try replacements.put(alloc, result_id, a);
                    },
                    143 => { // OpMatrixTimesScalar: mat * 1.0 = mat
                        if (float_one_ids.isSet(b)) try replacements.put(alloc, result_id, a);
                    },
                    else => {},
                }

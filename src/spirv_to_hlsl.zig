...
// scope. Threadlocal so emitWhileLoopHLSL/emitBlock/emitInstruction see the
// state without threading two more params through every call site (same
// pattern as the GLSL/MSL backends' g_loop_phis). See spirv_cross_common.zig.
...
// #cursor-warp: function locals that need definite initialization (see
// hlslDefiniteInitSet). The .Variable handler gives each of these a zero
// initializer so no emitted read can be an undef read.
threadlocal var g_definite_init_ids: ?*const std.AutoHashMap(u32, void) = null;
...
///
/// One exception: an id the #413 pre-scan hoisted above a loop is declared there by the
/// hoist, under whatever `names` holds for it, and the loop's carry copy reads that same
/// name. Handing such an id a `_phi` name splits the declaration from the use: in
/// graphicsfuzz_045 the hoist emitted `int v50;` and the carry `v39 = v50;` while the
/// merge materialization assigned `v77_phi`, so v50 was never written and DXC rejected
/// the loop. Mirrors mslPhiVarName, which has always had this exclusion.
fn hlslPhiVarName(names: *std.AutoHashMap(u32, []const u8), rid: u32, alloc: std.mem.Allocator) []const u8 {
    if (g_hoisted_ids) |h| {
        if (h.contains(rid)) {
...
fn emitMergePhiDeclsHLSL(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), label_map: *const std.AutoHashMap(u32, usize), mphis: []const HlslMergePhi, has_else: bool, tl: u32, mval: u32, w: anytype, alloc: std.mem.Allocator, indent: []const u8) !void {
    for (mphis) |pv| {
        ...
        // copy reads THAT variable. Redeclaring it here would shadow it, so the arm
        // copies below would write the inner one and the carry would read a variable
        // nothing ever assigned (graphicsfuzz_045: DXC "Loop must have break", because
        // the loop counter is carried through exactly such a phi). Drop the type and
        // keep the initializer -- skipping the line outright would leave the phi
        // undefined on the fall-through path. Mirrors the MSL emitter.
        const hoisted = if (g_hoisted_ids) |h| h.contains(pv.result_id) else false;
        const ty: []const u8 = if (hoisted) "" else t;
        ...
        if (has_else) {
            ...
            try w.print("{s}    {s}{s}{s} = {s};\n", .{ indent, ty, sep, vn, hlslExprName(module, names, false_val, alloc) });
        }
    }
}
...
fn finalizeSwitchPhisHLSL(names: *std.AutoHashMap(u32, []const u8), phis: []const Instruction, alloc: std.mem.Allocator) void {
    for (phis) |phi| {
        const pn = hlslPhiVarName(names, phi.words[2], alloc);
...
fn parseModule(alloc: std.mem.Allocator, words: []const u32) !ParsedModule {
    ...
    for (module.instructions) |inst| {
        ...
        // [numthreads] emits the intended workgroup size instead of silently 1x1x1.
...
pub fn spirvToHLSL(
    ...
    try w.writeAll("// Generated by zioshade SPIR-V -> HLSL cross-compiler\n\n");
...
fn hlslTextureTypeFromImage(module: *const ParsedModule, image_type_id: u32) []const u8 {
    ...
    if (is_storage) {
        if (is_int) |int_type| {
            ...
            const int_idx: usize = switch (int_type) {
                .int => 0,
                .uint => 1,
            };
            const dim_idx: usize = switch (dim) {
                .Dim1D => 0,
                .Dim2D => if (is_arrayed) 3 else 1,
                .Dim3D => 2,
                .DimBuffer => 4,
                else => 1,
            };
...
fn hlslEmitOneStructForwardDecl(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), type_id: u32, w: anytype, alloc: std.mem.Allocator, emitted: *std.AutoHashMap(u32, void), emitted_names: *std.StringHashMap(void)) !void {
    ...
    // every member rendered. An error mid-member-list (a spec-constant array
    ...
    for (inst.words[2..], 0..) |mt_id, mi| {
        ...
        const row_major_qual: []const u8 = blk: {
            if (matrix_tid) |mtid| {
                if (memberIsRowMajor(module, type_id, @intCast(mi))) {
                    // Explicit RowMajor. SQUARE: `row_major` reconstructs the
                    // logical M directly (HLSL rows = SPIR-V rows). NON-SQUARE:
                    // the FLIPPED keyword `column_major` is the sound spelling
                    // (SPIRV-Cross's convention, verified against its --hlsl
                    // output): row-major bytes are R chunks of C floats, which
                    // is exactly how HLSL column_major floatCxR (R columns of C
                    // floats) groups them, so the HLSL value is M^T -- the SAME
                    // transposed convention as the non-square ColMajor case
                    // below, and the existing use-site machinery (raw `m[i]`
                    // row reads in findUniformMatrixColumnAccess, operand swap
                    // in emitMatrixMulSwapped) applies unchanged.
                    break :blk if (matrixIsNonSquare(module, mtid)) "column_major " else "row_major ";
                }
        ...
        if (mti) |mi2| {
            if (mi2.op == .TypeArray and mi2.words.len > 3) {
                // Multi-dim array member (e.g. `T var[A][B]`, the SSBO-struct shape
                // struct-packing exercises as row_major mat2x3 m6[4][2]): collect
                // every nested TypeArray dimension like emitStructMembers does;
                // emitting only the outer one would declare a too-small struct
                // (wrong strides for every later member = silent-wrong layout).
                // Each level's size must resolve to a concrete value
                // (OpConstant / OpSpecConstant / OpSpecConstantOp arithmetic);
                // honest-error rather than emit a wrong/garbage dimension.
                var dims_buf: [64]u8 = undefined;
                var dims_len: usize = 0;
                var base_type_id: u32 = mi2.words[2];
                var cur: Instruction = mi2;
                while (true) {
                ...
                const et = try hlslType(module, base_type_id, names, alloc);
                var mname_buf: [32]u8 = undefined;
                const mname = hlslGetMemberName(module, type_id, @intCast(mi), &mname_buf);
                try bw.print("    {s}{s} {s}{s};\n", .{ row_major_qual, et, mname, dims_buf[0..dims_len] });
                continue;
            }
        }
...
fn emitStructMembers(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), struct_type_id: u32, cbuffer_name: []const u8, w: anytype, alloc: std.mem.Allocator) !void {
    ...
    for (inst.words[2..], 0..) |member_type_id, member_idx| {
        ...
        //     emits the FLIPPED keyword `column_major`: row-major bytes are R
        //     chunks of C floats, and HLSL column_major floatCxR groups exactly
        //     R columns of C floats, so the value is M^T (the same convention
        //     as non-square ColMajor above) and the raw `m[i]` / swapped-mul
        //     compensation applies unchanged -- SPIRV-Cross's convention.
        ...
        const row_major_qual: []const u8 = blk: {
            if (matrix_tid) |mtid| {
                if (memberIsRowMajor(module, struct_type_id, @intCast(member_idx))) {
                    // Explicit RowMajor. SQUARE keeps `row_major` (value = M).
                    // NON-SQUARE takes the flipped keyword `column_major` so the
                    // HLSL value is M^T, identical to the non-square ColMajor
                    // case below (see the block comment above for the byte
                    // grouping). SPIRV-Cross's convention for this shape.
                    break :blk if (matrixIsNonSquare(module, mtid)) "column_major " else "row_major ";
                }
...
fn emitFunction(
    ...
    // consumes N consecutive HLSL semantic slots (k..k+N-1), but zioshade keys the
    ...
    // real feature zioshade does not yet emit — so fail loud on a genuine overlap
...
/// Definite-assignment site: a block index plus the instruction index within the
/// function body slice, for ordering same-block reads against writes.
const InitSite = struct { block: usize, idx: usize };

/// The set of a function's locals that must be zero-initialized at declaration:
/// Function-storage OpVariables with no SPIR-V initializer whose value is not
/// definitely assigned at some read (a load can execute with no prior store).
///
/// Why: HLSL compilers lower such a read to a literal `undef` operand, and the
/// DXIL validator rejects the module with
///
///   error: Instructions should not read uninitialized value.
///   note: at '%2 = fadd fast float %1, undef' in block '#0' of function 'main'.
///   Validation failed.
///
/// (verified with the Windows SDK dxc 10.0.26100.0 and the pinned Linux DXC
/// 1.9.2602.24 used by `just hlsl-dxc`). A read folded straight into an output
/// only warns, but the same undef feeding arithmetic with a real operand is a
/// hard validation error, so the whole class must be closed, not the arithmetic
/// subset. The classic shape is the wintty cursor_warp gallery shader: a local
/// (or an array/struct cell) assigned only inside a conditional and read after;
/// spirv-cross's HLSL fails DXC identically there. spirv-cross's opt-in
/// --force-zero-initialized-variables is the blanket version of this fix; here
...
///    cursor_warp shape) IS initialized: the fall-through path reads undefined
///    in the source. DXC happens to accept that one as a phi with an undef
///    incoming, but the never-stored array cell and the read-before-branch
///    variants of the same shape are hard validation errors, so the contract is
///    definite assignment for the whole class, not the subset DXC rejects.
///  * Loop-carried reads (load at the header, store in the body) are initialized:
///    iteration 0 reads the never-stored local, undefined in the source too.
///  * A local referenced ONLY as a call argument is treated as a byref out-param
///    buffer, not a read (the emitted inout-arg temp would otherwise be
///    zero-initialized in every translated shader).
fn hlslDefiniteInitSet(alloc: std.mem.Allocator, module: *const ParsedModule, func_idx: usize) !std.AutoHashMap(u32, void) {
    var set = std.AutoHashMap(u32, void).init(alloc);
    errdefer set.deinit();

    // Function body slice (OpFunction .. OpFunctionEnd, exclusive).
    var func_end = module.instructions.len;
    var fe = func_idx + 1;
    while (fe < module.instructions.len) : (fe += 1) {
        if (module.instructions[fe].op == .FunctionEnd) {
            func_end = fe;
            break;
        }
    }
    const body = module.instructions[func_idx + 1 .. func_end];

    var locals = std.AutoHashMap(u32, void).init(alloc);
    defer locals.deinit();
    ...
    var uses = std.AutoHashMap(u32, std.ArrayListUnmanaged(InitSite)).init(alloc);
    defer {
            ...
            .AccessChain, .InBoundsAccessChain => {
                if (inst.words.len >= 4) {
                    ...
                    if (locals.contains(base)) {
                        try roots.put(inst.words[2], base);
                    } else if (roots.get(base)) |r| {
    ...
    // Definite-assignment dataflow over the same body (blocks numbered by
    // OpLabel appearance, the `block` counter above). Per local: `in[B]` is true
    // iff the local is definitely assigned on EVERY path from entry to B's start;
    // `out[B] = in[B] or any-def-in-B`. A read is covered iff a same-block def
    // precedes it or `in[its block]`. This is the textbook join (intersection)
    // over predecessors, so an if/else that assigns BOTH arms needs no
    ...
    for (succs, 0..) |sl, sb| {
        for (sl) |t| {
            ...
            if (!dup) try preds[t].append(alloc, sb);
        }
    }
    const reachable = try reachableBlocks(alloc, succs);
    ...
    while (lit.next()) |vid| {
        ...
        // has_def[B]: any store to this local in block B (any index).
        const has_def = try alloc.alloc(bool, nblocks);
        defer alloc.free(has_def);
        @memset(has_def, false);
        if (dlist) |ds| {
            for (ds.items) |d| has_def[d.block] = true;
        }

        const is_in = try alloc.alloc(bool, nblocks);
        defer alloc.free(is_in);
        @memset(is_in, true);
        is_in[0] = false;
        var changed = true;
        while (changed) {
            ...
            for (0..nblocks) |b| {
                if (b == 0) {
                    // Entry: nothing assigned before the function starts.
                    if (is_in[0] != false) {
                        is_in[0] = false;
                        changed = true;
                    }
        ...
        var needs = false;
        for (ulist.items) |u| {
            var covered = is_in[u.block];
            if (!covered) {
                if (dlist) |ds| {
                    for (ds.items) |d| {
                        if (d.block == u.block and d.idx < u.idx) {
                            covered = true;
                            break;
                        }
                    }
        ...
        if (needs) try set.put(vid.*, {});
    }

    return set;
}

/// Append a def/use site to a per-local list, initializing the list on first sight
...
/// Resolve a pointer operand to the Function-storage local it is rooted at, if any.
...
fn reachableBlocks(alloc: std.mem.Allocator, succs: [][]usize) ![]bool {
    const n = succs.len;
    const seen = try alloc.alloc(bool, n);
    defer alloc.free(seen);
    @memset(seen, false);
    const out = try alloc.alloc(bool, n);
...
fn emitBody(
    ...
    // loops with the OUTERMOST loop, whose scope covers the inner one.
    {
        ...
        while (li < module.instructions.len) : (li += 1) {
            ...
            for (plist.items) |pi| {
                ...
                if (!common.loopPhiUpdateNeedsHoist(module.instructions, module.id_defs, &label_map, &deferred_hdr, li, pi.update_id)) continue;
    ...
    // counter OpLoad living in a Pattern-B cond block, present when the counter is a
    // Function var used past the loop) is emitted in the body AFTER the carry, so the
    // carry reads it out of scope. HLSL rejects that (Metal tolerates it, which is why
    // prove_opt never surfaced it). OpPhi header values are pre-declared, so only NON-phi
    // header definitions need this. Mirrors the #413 phi-update hoist above.
    {
        var li = func_idx + 1;
        while (li < module.instructions.len) : (li += 1) {
            const minst = module.instructions[li];
    ...
    // Pattern-B replay puts the definition inside `while (true) { ... }`, a sibling
    // scope of loop 2. graphicsfuzz_072 has three sequential loops and computes its
    // exit-test operands in the first one's header.
    //
    ...
    // so the hoisted variable is assigned before any post-loop read. Port of the MSL
    // pass; kept structurally identical so the two do not drift.
    {
        var li = func_idx + 1;
        while (li < module.instructions.len) : (li += 1) {
            const minst = module.instructions[li];
            ...
            while (hi < li) : (hi += 1) {
                ...
                // not a post-loop read, the loop-merge-phi mechanism copies it inside the
                // loop where it is in scope.
                var ci = merge_idx;
                while (ci < module.instructions.len and (module.instructions[ci].op == .Label or module.instructions[ci].op == .Phi)) : (ci += 1) {}
                var referenced = false;
                while (ci < module.instructions.len) : (ci += 1) {
                    if (module.instructions[ci].op == .FunctionEnd) break;
                    const cw = module.instructions[ci].words;
                    var wi: usize = 1;
    ...
    while (idx < module.instructions.len) : (idx += 1) {
            ...
            if (phi_hdr.get(inst.words[2])) |lmi| {
                if (loop_phis.get(lmi)) |plist| {
                    for (plist.items) |pi| {
                        ...
                        // #413: an inner loop's phi variable can be an OUTER
                        // loop's carry source; its declaration was hoisted
                        // above the outer loop, so only assign here.
        ...
        if (inst.op == .BranchConditional) {
            ...
            if (merge_label) |ml| {
                ...
                // silent-wrong; fail loud. zioshade's own frontend always emits
...
fn emitWhileLoopHLSL(
    ...
    if (g_loop_hoists) |lh| {
        if (lh.get(loop_idx)) |hlist| {
            for (hlist.items) |h| {
                ...
                const tyname = try hlslType(module, h.type_id, names, alloc);
    ...
    if (next_inst.op == .Branch and next_inst.words.len >= 2) {
        ...
        if (common.detectDoWhileBackEdge(module, cont_lbl, header_lbl, merge_lbl, label_map)) |dw_bc| {
            is_do_while = true;
            bc_idx = dw_bc;
            cond_start = null;
        } else {
    ...
    if (is_do_while) {
        ...
        if (body_nested and !common.dowhileNestedBodyPhiSafe(module, loop_idx, merge_lbl, cont_lbl, label_map, true, true)) {
    ...
    if (has_phis and !dw_native) {
        ...
        if (cont_refs.count() > 0) {
            ...
            while (k < body_end and k < module.instructions.len) : (k += 1) {
                ...
                const phi_name = hlslPhiVarName(names, rid, alloc);
                try w.print("    {s} {s};\n", .{ rtt, phi_name });
        ...
        while (bi < module.instructions.len) : (bi += 1) {
                ...
                if (phi_hdr.get(binst.words[2])) |lmi| {
                    if (loop_phis.get(lmi)) |plist| {
                        for (plist.items) |pi| {
                            ...
                            const vname = names.get(pi.result_id) orelse "vphi";
                            const init_name = names.get(pi.init_id) orelse "0";
                            // #413: hoisted above the outer loop — assign only.
                            const hoisted = if (g_hoisted_ids) |h| h.contains(pi.result_id) else false;
                            if (hoisted) {
                                try w.print("        {s} = {s};\n", .{ vname, init_name });
                            } else {
            ...
            if (binst.op == .BranchConditional) {
                ...
                const tl_is_trivial_continue = blk: {
                    ...
                    if (tli + 2 < module.instructions.len and module.instructions[tli].op == .Label and module.instructions[tli + 1].op == .Branch and module.instructions[tli + 1].words.len > 1 and module.instructions[tli + 1].words[1] == cont_lbl) break :blk true;
                    break :blk false;
                };
                const fl_is_trivial_continue = blk: {
                    ...
                    if (fli + 2 < module.instructions.len and module.instructions[fli].op == .Label and module.instructions[fli + 1].op == .Branch and module.instructions[fli + 1].words.len > 1 and module.instructions[fli + 1].words[1] == cont_lbl) break :blk true;
                    break :blk false;
                };
                const tl_is_trivial_break = blk: {
                    if (ntl == merge_lbl) break :blk true;
                    const tli2 = label_map.get(ntl) orelse break :blk false;
                    if (tli2 + 2 < module.instructions.len and module.instructions[tli2].op == .Label and module.instructions[tli2 + 1].op == .Branch and module.instructions[tli2 + 1].words.len > 1 and module.instructions[tli2 + 1].words[1] == merge_lbl) break :blk true;
                    break :blk false;
                };
                const fl_is_trivial_break = blk: {
                    ...
                    if (fli2 + 2 < module.instructions.len and module.instructions[fli2].op == .Label and module.instructions[fli2 + 1].op == .Branch and module.instructions[fli2 + 1].words.len > 1 and module.instructions[fli2 + 1].words[1] == merge_lbl) break :blk true;
                    break :blk false;
                };
                if (nml) |nmv| {
...
fn emitInstruction(
    ...
    switch (inst.op) {
        .Variable => {
            ...
            // #cursor-warp: definite initialization. A local read where no store
            // dominates the read compiles to an undef operand in DXIL, which the
            // validator rejects ("Instructions should not read uninitialized
        ...
        // (verified exhaustively). Componentwise for vectors. spirv-cross's own MSL backend
        ...
        .OuterProduct => {
            ...
            // constructs HLSL matrices column-by-column (floatRxC(col0, col1, …),
            // the same convention CompositeConstruct uses above), so passing each
            // v1 * v2[j] as constructor arg j is exact. Column count = v2's
            // component count. Without this arm it fell through to `// unhandled op
            // 147`, leaving the result id undefined (DXC-rejected, silent-wrong).
            const rt = try hlslType(module, inst.words[1], names, alloc);
            const v1 = names.get(inst.words[3]) orelse "a";
            const v2 = names.get(inst.words[4]) orelse "b";
            const v2_def = getDef(module, inst.words[4]);
            const v2_ty = if (v2_def) |d| (if (d.words.len > 1) getDef(module, d.words[1]) else null) else null;
        ...
        .FOrdGreaterThanEqual, .SGreaterThanEqual, .UGreaterThanEqual => try common.emitBinOp(module, names, inst, ">=", w, alloc, hlslType),
        ...
        // emits). See emitNegatedCompare. OpFUnordEqual is TRUE on NaN too and has no
        // ordered complement (`!=` is true on NaN), so it goes through emitUnordEqual
        // (isnan(a)||isnan(b)||(a==b)). OpFUnordNotEqual is already exact on `!=`.
        ...
        .CompositeExtract => {
            ...
            // Accumulate the expression into a buffer (instead of writing it
            // straight to `w`) because extracting a COLUMN out of a
            // buffer-sourced SQUARE matrix must wrap the sub-expression in
            // transpose(...) mid-chain: a square cbuffer matrix is stored as
            // the LOGICAL M (bare column_major, see emitStructMembers), while
            // every other matrix value (locals, non-square buffer matrices)
            // is stored M^T; SPIR-V OpCompositeExtract selects a COLUMN but
            // HLSL `m[n]` is ROW n, so only the square-buffer case needs
            // `transpose(m)[n]`. Mirrors findUniformMatrixColumnAccess, which
            // compensates the same read reached via OpAccessChain instead of
            // load + extract (the matrixindex.flatten.vert shape).
            var expr_buf = std.ArrayList(u8).initCapacity(alloc, 64) catch return error.OutOfMemory;
            defer expr_buf.deinit(alloc);
            try expr_buf.appendSlice(alloc, comp);
            ...
            for (inst.words[4..]) |index| {
                ...
                if (is_current_vec) {
                    try expr_buf.appendSlice(alloc, switch (index) {
                        0 => ".x",
                        1 => ".y",
                        2 => ".z",
                        3 => ".w",
                    ...
                    if (current_parent) |pt| {
                        ...
                        if (pt_inst) |pi| {
                            if (pi.op == .TypeStruct) {
                                ...
                                try expr_buf.print(alloc, ".{s}", .{mname});
                                used_name = true;
                            } else if (pi.op == .TypeMatrix) {
                                // SPIR-V OpCompositeExtract/Insert on a matrix
                                // selects a column (column-major). zioshade emits
                                // matrices column-by-column, so HLSL `m[n]` on an
                                // M^T-convention value yields that same stored
                                // column vector. `._mN` is invalid HLSL matrix
                                // syntax and DXC rejects it.
                                if (matrixIsBufferSourced(module, inst.words[3]) and
                                    !matrixIsNonSquare(module, pt))
                                {
                                    // Square buffer matrix: the value holds the
                                    // logical M, so transpose before the row read
                                    // (see the expr_buf comment above).
                                    var wrapped = std.ArrayList(u8).initCapacity(alloc, expr_buf.items.len + 12) catch return error.OutOfMemory;
                                    defer wrapped.deinit(alloc);
                                    try wrapped.appendSlice(alloc, "transpose(");
                                    try wrapped.appendSlice(alloc, expr_buf.items);
                                    try wrapped.appendSlice(alloc, ")");
                                    expr_buf.clearRetainingCapacity();
                                    try expr_buf.appendSlice(alloc, wrapped.items);
                                }
                                try expr_buf.print(alloc, "[{d}]", .{index});
                                used_name = true;
                    ...
                    if (!used_name) try expr_buf.print(alloc, "._m{d}", .{index});
                }
            ...
            try w.print("    {s} {s} = {s};\n", .{ rt, names.get(inst.words[2]) orelse "v", expr_buf.items });
        },
        .CompositeInsert => {
            const rt = try hlslType(module, inst.words[1], names, alloc);
            const rname = names.get(inst.words[2]) orelse "v";
        ...
        // Runtime SSBO array `.length()` (OpArrayLength). zioshade flattens a single-
...
/// True if `type_id` is a NON-square matrix (column count != row count). Both
/// non-square storage flavors (ColMajor default and explicit RowMajor) are
/// emitted with a FLIPPED HLSL keyword (`row_major` / `column_major`) so the
/// HLSL value is M^T -- see emitStructMembers and emitMatrixMulSwapped for the
/// byte-grouping proof and the use-site compensation.
...
/// is the column/element tail). zioshade stores a cbuffer matrix as the logical
...
/// `mul` and must stay untransposed -- and for non-square matrices. EVERY
/// non-square buffer matrix is stored as M^T by emitStructMembers (ColMajor
/// gets `row_major`, explicit RowMajor gets `column_major` -- the flipped
/// keyword either way), so an HLSL row read `var[i]` already yields column i of
/// M (M^T's row i = M's column i): no transpose is wanted, and the null return
/// here (raw access) is therefore CORRECT for non-square.
    ...
    if (ssbo_elem_type) |elem_type| {
        ...
        for (indices[@min(indices.len, 2)..]) |index_id| {
            ...
            if (idx_inst) |def| {
                if (def.op == .Constant and def.words.len > 3) {
                    const val = def.words[3];
                    const is_vector = if (cur) |tid| blk2: {
                        const ti = getDef(module, tid);
                        break :blk2 ti != null and ti.?.op == .TypeVector;
                    } else false;
                    if (is_vector) {
                        try buf.appendSlice(alloc, switch (val) {
                            0 => ".x",
...
/// sign combination. Componentwise for vectors. (spirv-cross's MSL uses an equivalent
/// spvSMod helper.)
fn emitSMod(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), inst: Instruction, w: anytype, alloc: std.mem.Allocator) !void {
    const rt = try hlslType(module, inst.words[1], names, alloc);
...
/// with the FLIPPED keyword by emitStructMembers (`row_major` for ColMajor,
/// `column_major` for explicit RowMajor -- bare column_major would misgroup the
/// ColMajor bytes), so HLSL holds it as M^T -- the SAME transposed convention as a
/// local matrix -- and the mul operands MUST be swapped. Construction, indexing
/// (`m[i]`), transpose() and the spvInverseNxN helper operate on the stored
/// representation and stay consistent; only
...
/// ColMajor case matches spirv-cross (row_major + swapped operands) and is DXC-verified
/// (#497); the non-square explicit RowMajor case is the same M^T convention with the
/// opposite keyword, mirroring spirv-cross's `column_major` spelling.
fn emitMatrixMulSwapped(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), inst: Instruction, w: anytype, alloc: std.mem.Allocator) !void {
    ...
    // A non-square buffer matrix is emitted with a FLIPPED keyword (row_major for
    // ColMajor, column_major for explicit RowMajor) by emitStructMembers, so it
    // uses the SAME transposed convention (M^T) as a local matrix and the operands
...
fn emitStd450(module: *const ParsedModule, names: *std.AutoHashMap(u32, []const u8), inst: Instruction, instruction: u32, w: anytype, alloc: std.mem.Allocator) !void {
    ...
    // reflect(...)/refract(...). scalar-refract-reflect in the spirv-cross corpus.
...
/// u32 -> GLSLstd450 adapter: the shared do-while cluster (spirv_cross_common)

...
fn compileToSpirv(name: []const u8, source: [:0]const u8) ![]u32 {
    const tmp_src = try zioshade.compat.tempFilePathFmt(alloc, "glsl_test_{s}.frag", .{name});
    defer alloc.free(tmp_src);
    const tmp_spv = try zioshade.compat.tempFilePathFmt(alloc, "glsl_test_{s}.spv", .{name});
...
// #170 OpFUnordEqual: unordered-equal is TRUE on a NaN operand (ordered `==`/`equal`
// is false there = plausible-but-wrong). Unlike the inequalities it has no ordered
// complement (`!=` is true on NaN), so GLSL lowers it to isnan(a)||isnan(b)||(a==b)
// (scalar) and a per-component bvec constructor (vector), since GLSL has no
// componentwise || on bvec. Deliberate divergence from spirv-cross (which emits
// `!(a != b)`, NaN-wrong). FOrdEqual (180) is rewritten to FUnordEqual (181).
test "glsl: OpFUnordEqual lowers NaN-correctly (isnan||isnan||==) (#170)" {
    // scalar: isnan(a) || isnan(b) || (a == b)
    const s_spv = try compileToSpirv("funord_eq_s",
        \\#version 450
        \\layout(location = 0) in float a;
        \\layout(location = 1) in float b;
        \\layout(location = 0) out vec4 o;
    ...
    defer alloc.free(s_spv);
    const s_rw = try rewriteFirstOpcode(s_spv, 180, 181); // FOrdEqual -> FUnordEqual
    defer alloc.free(s_rw);
    const s_glsl = try zioshade.spirvToGLSL(alloc, s_rw, .{ .version = 430 });
    defer alloc.free(s_glsl);
    try assertContains(s_glsl, "isnan(");
    try assertContains(s_glsl, "|| (");
    try glslValidateOrSkip("funord-eq-scalar", s_glsl);

    // vector: per-component bvec3(isnan(a).x||isnan(b).x||(a.x==b.x), ...)
    const v_spv = try compileToSpirv("funord_eq_v",
        \\#version 450
        \\layout(location = 0) in vec3 a;
        \\layout(location = 1) in vec3 b;
        \\layout(location = 0) out vec4 o;
        \\void main() { bvec3 c = equal(a, b); o = vec4(float(c.x), float(c.y), float(c.z), 1.0); }
    );
    defer alloc.free(v_spv);
    const v_rw = try rewriteFirstOpcode(v_spv, 180, 181);
    defer alloc.free(v_rw);
    const v_glsl = try zioshade.spirvToGLSL(alloc, v_rw, .{ .version = 430 });
    defer alloc.free(v_glsl);
    try assertContains(v_glsl, "isnan(");
    try assertContains(v_glsl, ".x == "); // per-component equality, not bare equal()
...
// #170: textureProjLodOffset → GLSL native textureProjLodOffset(s, coord, lod, offset).
// The proj-explicit-lod Lod arm must spell the offset form, not a plain textureProjLod.
test "glsl: textureProjLodOffset keeps the const offset (#170)" {
    const source =
        \\#version 450
        \\layout(binding=0) uniform sampler2D s;
    ...
    const glsl = try compileToGlsl(source);
    defer alloc.free(glsl);
    try assertContains(glsl, "textureProjLodOffset(");
...
test "glsl: textureProjGradOffset keeps the const offset (#170)" {
    ...
    const glsl = try compileToGlsl(source);
    defer alloc.free(glsl);
    try assertContains(glsl, "textureProjGradOffset(");
    try assertContains(glsl, "ivec2(2, -1)");
}
...
test "glsl: textureProjGrad lowers to native textureProjGrad (#170)" {
    ...
    const glsl = try compileToGlsl(source);
    defer alloc.free(glsl);
    try assertContains(glsl, "textureProjGrad(");
    try assertNotContains(glsl, "textureProjLod(");
}
...
// the param INTO the local, not a write-back. The param was emitted as `out`
// (argument value never arrives), the local was aliased to the param name
// (redeclaration = hard compile error on glslang), and the entry copy
// degenerated to a self-assign of an undefined value.
test "glsl: in param seeding a written-then-read local stays a value param (#414)" {
    const source =
        \\#version 430
    ...
    const glsl = try compileToGlsl(source);
    defer alloc.free(glsl);
    try assertContains(glsl, "float map(float ");
    try assertNotContains(glsl, "map(out ");
    const open = (std.mem.indexOf(u8, glsl, "float map(float ") orelse return error.TestExpectedFind) + "float map(float ".len;
    const close = std.mem.indexOfScalarPos(u8, glsl, open, ')') orelse return error.TestExpectedFind;
    const param = std.mem.trim(u8, glsl[open..close], " ");
    const redecl = try std.fmt.allocPrint(alloc, "float {s};", .{param});
    defer alloc.free(redecl);
    try assertNotContains(glsl, redecl);
    const selfassign = try std.fmt.allocPrint(alloc, "{s} = {s};", .{ param, param });
    defer alloc.free(selfassign);
    try assertNotContains(glsl, selfassign);
    const copy = try std.fmt.allocPrint(alloc, " = {s};", .{param});
    defer alloc.free(copy);
    try assertContains(glsl, copy);
}
...
test "glsl: read-only param shadowed by a mutated inner local is not promoted (#414)" {
    const source =
        \\#version 430
        \\layout(binding = 0, std140) uniform U { float t; } ub;
        \\layout(location = 0) out vec4 o;
    ...
    const glsl = try compileToGlsl(source);
    defer alloc.free(glsl);
    try assertContains(glsl, "float f(float ");
    try assertNotContains(glsl, "f(out ");
}
...
/// was declared later in the body (`float v106 = v98 + v103;`) — glslang
/// rejects the output with "undeclared identifier". A declaration site is an
/// occurrence whose previous token is another identifier (its type); statement
/// keywords are excluded so `return v5;` counts as a use.
...
test "glsl: loop-carried phi update defined in the body is declared before the loop top reads it (#413)" {
    const source =
        \\#version 430
...
test "glsl: raymarcher with inlined-function loops has no use-before-declaration (#413)" {
    const source =
        \\#version 430
        \\layout(binding = 0, std140) uniform U { vec3 res; float time; } ub;
        \\layout(location = 0) out vec4 _fragColor;

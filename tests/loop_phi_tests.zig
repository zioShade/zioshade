...
test "MSL materializes a top-level loop-merge phi with a trivial break block (#loop-merge-phi)" {
    ...
    // break arm's copy (%brkval -> v17) before its `if (c) break;`; the post-loop
    // read uses the carrier. Fixture reconstructed 2026-08-16 (the #635 squash
    // never committed it); render-verified MATCH vs naga.
...
test "HLSL emits the break for a side-effecting break block inside a selection (#loop-break-on-selection-merge)" {
    ...
    // Fixture reconstructed 2026-08-16 (names pinned to its actual emission:
    // counter v15, computed value v18); render-verified MATCH vs naga.

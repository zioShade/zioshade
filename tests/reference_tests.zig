...
fn testShader(name: []const u8, source: [:0]const u8) !void {
    ...
    const msl: ?[]const u8 = zioshade.spirvToMSL(alloc, spirv, .{}) catch |err| blk: {
        if (err == error.UnsupportedFragmentOutput or err == error.UnsupportedOpcode or err == error.UnsupportedMultiSampleStorageImage or err == error.UnsupportedComponentPacking) break :blk null;

...
A hand-written UBO (`.spvasm`) whose struct type AND instance are both
`OpName "Globals"` (the instance is NAMED, not anonymous), plus a function-local
also `OpName "Globals"`. Hand-written because glslang emits ANONYMOUS UBO
instances, whose collision-set name is `""` -- the local would never collide and
the block-instance exclusion would go unexercised.
The instance is a GLSL UBO (Uniform + `Block`, not `BufferBlock`), so it is
emitted under a block-naming-mangled name (`Globals_1`), NOT its raw OpName; a
local sharing the block's raw OpName therefore does NOT shadow it. The fix
EXCLUDES such UBO instances from the collision set, so the local keeps `Globals`
(it must NOT be mangled to `Globals_1`, which would collide with the instance
and break the UBO access). This guards against the regression a broader

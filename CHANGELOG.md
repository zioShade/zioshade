...
- **WGSL coverage deepening, Pass 1 (G5, #170):** three classes of WGSL backend defects, all naga-gated:
  ...
  - **Projective texture sampling was silent-wrong:** `textureProj*` (ImageSampleProj{Implicit,Explicit,DrefImplicit,DrefExplicit}Lod) lowered to a fabricated manual perspective divide `textureSample(t, s, coord.xy / coord.w)` — value-sensitive wrong (it ignored the dref/lod/bias operands and diverged from GLSL semantics) yet passed naga. WGSL has no projective sampling builtin, so all four opcodes now **fail loud** with `error.UnsupportedOp` ("WGSL has no projective texture sampling") instead of fabricating output.

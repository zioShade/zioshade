...
> ZIOSHADE_BENCH_GLSLANG="C:/VulkanSDK/1.4.341.1/Bin/glslangValidator.exe" \
> ZIOSHADE_BENCH_SPIRVX="C:/VulkanSDK/1.4.341.1/Bin/spirv-cross.exe" \
>   zig build bench-compare
> ```
...
| Shader | zioshade avg | zioshade min | reference avg | reference min | **speedup** | HLSL bytes zioshade / reference |
|---|---:|---:|---:|---:|---:|---:|
...
**Highlights**
- zioshade is **150–265×** faster on this workflow than spawning `glslangValidator + spirv-cross`.
- zioshade's HLSL output is **5–47% smaller** on every shader tested — same source compiles to fewer bytes, despite zioshade's optimizer being newer than SPIRV-Cross's.
- Min vs avg confirms steady-state behaviour: zioshade's variance is sub-millisecond; reference variance is in the tens of milliseconds (mostly Windows process-spawn jitter).
...
|---|---|
| **zioshade (in-process Zig library)** | GLSL parse + SPIR-V codegen + optimization passes + HLSL emit. No process spawn, no DLL init, no IPC. |
...
- Output size is measured on the final HLSL string (zioshade returns null-terminated, reference returns stdout).
- zioshade build mode: `ReleaseFast`. Reference toolchain: stock Vulkan SDK 1.4.341.1 (LunarG).
...
2. **No DLL/init overhead.** glslang's C++ runtime initialization, locale setup, and resource-limit defaults happen once per process. zioshade is just function calls.
3. **Tight ID compaction.** zioshade's SPIR-V emitter compacts result IDs aggressively, and the cross-compilers inline single-use expressions. The combination yields visibly smaller HLSL on the same input.
4. **Single allocator domain.** Each call uses a single Zig allocator; no IPC, no string serialization.

## What this means in practice

For wintty's startup sequence (10 shaders): subprocess approach would take ~1.8 s before the terminal renders a frame. zioshade turns that into ~10 ms. That's the original motivation — a real engineering constraint, not a microbenchmark.
...
model. Stages zioshade does not yet emit valid HLSL for (vertex, raygen,
...
- The three `SV_Barycentrics` failures (`barycentric-{khr,khr-io-block,nv}.spv`)
  ...
  intentional dialect failures, not zioshade bugs.

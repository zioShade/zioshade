...
| **HLSL** | **compile-verified** (glslang + DXC) | glslang accepts the emitted HLSL (0 INVALID); DXC canonical SM6.x compile-verify now provisioned in Docker — 51 PASS / 3 honest-error / 2 SKIP; the D3D12 *render*-diff (semantic truth) is still founder-gated. **1 known WARP-confirmed defect**: an HLSL matrix-convention bug (`floatCxR` row-fill vs MSL col-fill → `mul(M,v)`=M^T·v; 3 RENDER-DIFFERs) — see `src/spirv_to_hlsl.zig` + `tools/warp/README.md`; its fix is locally unverifiable (the macOS proxy is blind to it) and awaits a WARP-verified pass | `just hlsl-glslang-all`, `just hlsl-dxc` |

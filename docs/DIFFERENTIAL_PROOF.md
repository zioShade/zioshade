...
| **WGSL** | **render-verified (proxy)** | naga compile (0 REJECT) + Metal-reuse render proxy (zioshade WGSL → naga → MSL → Metal vs MSL_ref): **1138 MATCH / 28 DIFFER (flagged)** — `#loop-continue-deadincr` (`continuing{}`) dropped 60 → 29 and `#switch-fallthrough` (statement duplication — `fallthrough` was removed from WGSL) dropped 29 → 28; residual is chaotic-FP / control-flow. Same single-oracle-proxy caveat | `just wgsl-naga`, `bash tools/wgsl_render_check.sh` |

...
| **GLSL** | **render-verified (proxy)** | glslang compile (0 INVALID) + Metal-reuse render proxy (zioshade GLSL → glslang → MSL → Metal vs MSL_ref): **1141 MATCH / 25 DIFFER (flagged)** — the `#loop-continue-deadincr` fix dropped DIFFER 70 → 26 and the `#switch-fallthrough` fix (port of HLSL's `caseTerminatorTarget`) dropped 26 → 25; residual is chaotic-FP / control-flow / harness artifacts. Single-oracle proxy, same correlated-normalization caveat as HLSL | `just glsl-glslang-all`, `bash tools/glsl_render_check.sh` |
| **WGSL** | **render-verified (proxy)** | naga compile (0 REJECT) + Metal-reuse render proxy (zioshade WGSL → naga → MSL → Metal vs MSL_ref): **1137 MATCH / 29 DIFFER (flagged)** — the `#loop-continue-deadincr` fix (WGSL `continuing{}` idiom) dropped DIFFER 60 → 29; residual is chaotic-FP / control-flow / `switch_fallthrough`. Same single-oracle-proxy caveat | `just wgsl-naga`, `bash tools/wgsl_render_check.sh` |

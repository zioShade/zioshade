...
| **HLSL** | **compile-verified** (glslang + DXC) | glslang accepts the emitted HLSL (0 INVALID); DXC canonical SM6.x compile-verify now provisioned in Docker — 51 PASS / 3 honest-error / 2 SKIP; the D3D12 *render*-diff (semantic truth) is still founder-gated. The previously WARP-flagged matrix-convention bug (`floatCxR` row-fill → transpose) is **addressed via `emitMatrixMulSwapped`** (mirrors spirv-cross's `mul(v,M)` convention) — zioshade's HLSL now byte-matches spirv-cross on the flagged shaders (mat3_branch, mat_cond_swizzle, outer_product_test); a WARP re-run is the proxy-blind confirmation, not a fix | `just hlsl-glslang-all`, `just hlsl-dxc` |

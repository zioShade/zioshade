...
validate-dxc: generate-outputs
    ...
    # Both crt_output.hlsl and focus_output.hlsl are regenerated fresh by
    # generate-outputs, so this gate validates current compiler output rather
    # than a committed snapshot that could silently drift.
...
# regenerate saved HLSL outputs from wintty shaders (crt + focus)
generate-outputs:
    {{zig}} run -ODebug --dep glslpp -Mroot=tools/dump_crt_hlsl.zig -Mglslpp=src/root.zig
    {{zig}} run -ODebug --dep glslpp -Mroot=tools/dump_shader.zig -Mglslpp=src/root.zig -- tests/wintty/shadertoy_prefix.glsl tests/wintty/test_focus.glsl tests/wintty/focus_output

...
- **A real-browser render gate for the WGSL backend.** `just wgsl-browser` (`tools/wgsl_browser_check.mjs`) compiles each corpus shader to WGSL, executes it in Chrome's actual WebGPU stack (tint + Dawn, rasterized by SwiftShader) through a harness page built on the wintty player's binding contract, screenshots the canvas, and pixel-diffs against the same Metal-proxy reference the render proxies use, all at one fixed deterministic uniform state (mid-animation cursor move, the state that makes the mode-change shaders execute their bug shapes). This is the only gate that catches the class where tint rejects at pipeline build what naga accepts (the zioshade-8k2 bug: the wintty.io player silently fell back to black while every text and proxy gate passed; the standing repro is pinned in `tools/wgsl_browser_fixtures/` and reported as REJECT with the verbatim tint message) and the class where the WGSL runs but renders wrong. On the vendored wintty gallery it is 16/16: 14 shaders render byte-tight against Metal (max channel diff <= 1) and the two divergent ones are triaged cross-GPU transcendental precision on CHAOS shaders (same structure, intensity-only diffs, documented in `tools/wgsl_browser_baseline.txt`). Manual-only: it needs Chrome for Testing, a playwright import, and the local Metal oracles, so it is not in `ci`/`ci-full`.
...
**Version policy?** SemVer on the public API exported from `src/root.zig`; the current version is the `.version` field in [`build.zig.zon`](build.zig.zon), which is the single source of truth. Pre-1.0, so minor bumps may break API. Builds on Zig 0.15.2 through 0.16 (see [Supported Zig versions](#supported-zig-versions)).
